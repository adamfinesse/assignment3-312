...
globalbuffer dequeuebuffer(int* sharedmemint) {
    if (*sharedmemint > 0) {// change this check for something not sure yet
    //printf("dequing");
        //fifo_index = (fifo_index+1)%30;
        //printf("fifo_index = %d\n",fifo_index);
       // printf("value here %d\n",gloalbufferarray[(*sharedmemint)].bytes);
        return gloalbufferarray[--(*sharedmemint)];
    } else {
...
void *consumer(void *thread_n) {
    ...
    sleep(8);
     while(true){
        ...
        value = dequeuebuffer(intsharedmem);
